...
class UserChatbotController extends Controller
{
    ...
    {
        return view('panel.user.chatbot.setting', [
            'title' => trans('Floating Chat Settings'),
            'method' => 'post',
            'action' => route('dashboard.user.chatbot.setting'),
    ...
    {   
        return view('panel.user.chatbot.index', [
            'title' => trans('Chatbot Training'),
            'items' => Chatbot::where('user_id', auth()->user()->id)->paginate(10)
        ]);
    }
    ...
    public function create()
    {
        ...
        return to_route('dashboard.user.chatbot.show', $item->getAttribute('id'));
    }
    ...
    public function store(ChatbotRequest $request)
    {
        ...
        return to_route('dashboard.user.chatbot.index')->with('success', trans('Chatbot Template Created Successfully'));
    }

    public function show(Chatbot $chatbot)
    {
        return view('panel.user.chatbot.training', [
            'title' => trans('Chatbot Training'),
            'item' => $chatbot,
            'data' => $chatbot->data()->get(),
            'action' => route('dashboard.user.chatbot.update', $chatbot),
        ]);
    }

    public function edit(Chatbot $chatbot)
    {
        return view('panel.user.chatbot.form', [
