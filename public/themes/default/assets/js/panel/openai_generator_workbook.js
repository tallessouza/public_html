...
const generate = async (message_no, creativity, maximum_length, number_of_results, prompt, openai_id) => {
	...
	if (prompt.includes("Aqui está sua transcrição")) {
		formattedText = md.render(md.utils.unescapeAll(prompt.replace(/<div>|<\/div>/g, '').replace(/<br>|<br\/>/g, '\n')));
	...
	if (stream_type == 'backend') {
		...
		messages.push({
			role: 'system',
			content: 'Você é um assistente atencioso e prestativo.'
		});
		messages.push({
			role: 'user',
			content: prompt
		...
		try {
			...
			switch (error.status) {
				case 429:
					toastr.error(magicai_localize?.api_connection_error || 'Erro de conexão com a API');
					break;
				default:
					toastr.error(magicai_localize?.api_connection_error_admin || 'Erro de conexão com a API');
			}
