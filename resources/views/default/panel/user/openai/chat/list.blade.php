...
@section('titlebar_actions')
    <x-button
        ...
        href="{{ $app_is_demo ? '#' : LaravelLocalization::localizeUrl(route('dashboard.user.openai.chat.addOrUpdate')) }}"
    >
        {{ __('Add Template') }}
    </x-button>
@endsection
                        ...
                        <td class="whitespace-nowrap text-end">
                            ...
                            @else
                                <x-button
                                    ...
                                    variant="ghost-shadow"
                                    size="none"
                                    href="{{ LaravelLocalization::localizeUrl(route('dashboard.user.openai.chat.addOrUpdate', $entry->id)) }}"
                                    title="{{ __('Edit') }}"
                                >
                                    <x-tabler-pencil class="size-4" />
                                </x-button>
                                <x-button
                                    ...
                                    hover-variant="danger"
                                    size="none"
                                    href="{{ LaravelLocalization::localizeUrl(route('dashboard.user.openai.chat.delete', $entry->id)) }}"
...
@push('script')
    <script src="{{ custom_theme_url('/assets/js/panel/openai.js?v='. time()) }}"></script>
@endpush
