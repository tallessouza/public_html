...
<input id="createChatUrl" type="hidden" name="createChatUrl" value="/dashboard/user/openai/webchat/start-new-chat" />
@endif
...
<div class="py-10">
    ...
    <div class="chats-wrap relative grid h-[calc(100vh-7rem)] grid-flow-row md:h-[75vh] md:grid-flow-col md:[grid-template-columns:25%_75%]" id="user_chat_area" x-data="{ mobileOptionsShow: false, toggleMobileOptions() { this.mobileOptionsShow = !this.mobileOptionsShow }, mobileSidebarShow: false, toggleMobileSidebar() { this.mobileSidebarShow = !this.mobileSidebarShow } }">
        ...
        @endif
        <x-card class="conversation-area-wrap flex h-[inherit] grow flex-col md:rounded-s-none lg:w-full" class:body="h-full rounded-b-[inherit] rounded-t-[inherit]" id="load_chat_area_container" size="none">
            @if ($chat != null)
...
<template id="chat_user_image_bubble">
    <div class="lqd-chat-image-bubble mb-2 flex flex-row-reverse content-end gap-2 lg:ms-auto">
        <div class="mb-2 flex w-4/5 justify-end rounded-3xl text-heading-foreground dark:text-heading-foreground md:w-1/2">
            <a data-fslightbox="gallery" data-type="image" href="#">
                <img class="img-content rounded-3xl" loading="lazy" />
            </a>
        </div>
    </div>
</template>
...
<template id="chat_user_bubble">
    <div class="lqd-chat-user-bubble mb-2 flex flex-row-reverse content-end gap-2 lg:ms-auto">
        <span class="size-6 inline-block shrink-0 rounded-full bg-cover bg-center" style="background-image: url({{ custom_theme_url(Auth::user()->avatar, true) }})">
            <span class="sr-only">
                @lang('You'):
            </span>
        </span>
        <div class="chat-content-container group relative max-w-[calc(100%-64px)] rounded-[2em] bg-secondary text-secondary-foreground dark:bg-zinc-700 dark:text-primary-foreground">
            <div class="chat-content px-6 py-3"></div>
            <button class="lqd-clipboard-copy size-10 pointer-events-auto invisible absolute -start-5 bottom-0 inline-flex items-center justify-center rounded-full border-none bg-background text-foreground opacity-0 shadow-lg transition-all hover:-translate-y-0.5 hover:scale-110 group-hover:visible group-hover:opacity-100" data-copy-options='{ "content": ".chat-content", "contentIn": "<.chat-content-container" }' title="{{ __('Copy to clipboard') }}">
...
<template id="chat_ai_bubble">
    <div class="lqd-chat-ai-bubble group mb-2 flex content-start gap-2">
        <span class="size-6 inline-block shrink-0 rounded-full bg-cover bg-center" style="background-image: url('{{ !empty($chat->category->image) ? custom_theme_url($chat->category->image, true) : custom_theme_url('assets/img/auth/default-avatar.png') }}')">
            <span class="sr-only">
                @lang('AI Assistant'):
            </span>
        </span>
        <div class="chat-content-container min-h-11 relative max-w-[calc(100%-64px)] rounded-3xl text-heading-foreground before:absolute before:inset-0 before:inline-block before:rounded-3xl before:bg-clay group-[&.loading]:before:animate-pulse-intense dark:text-heading-foreground dark:before:bg-white/[2%]">
            ...
            <div class="inline-flex max-w-full items-center rounded-full font-medium leading-none">
                ...
                @endif
                <pre class="chat-content prose relative w-full max-w-none whitespace-pre-wrap px-6 py-3 indent-0 font-[inherit] text-xs font-normal text-current [word-break:break-word] empty:hidden [&_*]:text-current"></pre>
                <button class="lqd-clipboard-copy size-10 pointer-events-auto invisible absolute -end-5 bottom-0 inline-flex items-center justify-center rounded-full border-none bg-background text-foreground opacity-0 shadow-lg transition-all hover:-translate-y-0.5 hover:scale-110 group-hover:visible group-hover:opacity-100" data-copy-options='{ "content": ".chat-content", "contentIn": "<.chat-content-container" }' title="{{ __('Copy to clipboard') }}">
                    <x-tabler-copy class="size-4" />
                </button>
            </div>
        </div>
...
<input id="guest_id" type="hidden" value="{{ $apiUrl }}">
<input id="guest_search" type="hidden" value="{{ $apiSearch }}">
<input id="guest_search_id" type="hidden" value="{{ $apiSearchId }}">
...
@else
<input id="prompt_prefix" type="hidden" value="">
@endif
@endsection
@push('script')
<link rel="stylesheet" href="{{ custom_theme_url('/assets/libs/prism/prism.css') }}">
